...
public class Calculator {
  ...
  protected int getPieceOfDividendWhichIsBiggerThenDivisorStartingFromZeroPosition(int dividend,
      int divisor) {
  ...
  protected void calculateNewDividendAndQuotient(int dividend, int divisor) {
    ...
    newDividend = NumbersOperations.concatenateTwoNumbersToOne(remain,
        getDigitByPosition(dividend, currentDigitPositionInDividend));

    while (newDividend < divisor && isDivisionAllowed(dividend)) {
      newDividend = NumbersOperations.concatenateTwoNumbersToOne(newDividend,
          getDigitByPosition(dividend, currentDigitPositionInDividend));
  ...
  protected DivisionVector calculateDivisionOffset(int dividend, int divisor) {
    ...
    while (isDivisionAllowed(dividend)) {
      ...
      divisionVectorOffsets.divisionResultOffset =
          currentDigitPositionInDividend - NumbersOperations.countOfDigitsInNumber(
              divisor * nextDigitOfQuotient);

      calculateNewDividendAndQuotient(dividend, divisor);

      divisionVectorOffsets.subtractionResultOffset =
          currentDigitPositionInDividend - NumbersOperations.countOfDigitsInNumber(newDividend);

      result.add(new int[]{divisor * nextDigitOfQuotient, newDividend,
          divisionVectorOffsets.divisionResultOffset,
          divisionVectorOffsets.subtractionResultOffset});
    }
  ...
  protected int calculateSubtractionOffsettBasedOnRemain(int dividend, int divisor,
      int lastNumBeforeRemainder, DivisionVector divisionVectorOffsets) {
    if (remain == 0) {
      if (dividend / divisor == 1 && dividend % divisor == 0) {
        return NumbersOperations.countOfDigitsInNumber(dividend) - 1;
      } else {
        return divisionVectorOffsets.divisionResultOffset + NumbersOperations.countOfDigitsInNumber(
            lastNumBeforeRemainder) - 1;
      }
    } else {
  ...
  protected ArrayList<int[]> solvingTheProblemAndConvertingItToArray(int dividend, int divisor) {
    newDividend = getPieceOfDividendWhichIsBiggerThenDivisorStartingFromZeroPosition(dividend,
        divisor);
    ...
    nextDigitOfQuotient = quotientAndRemaining.quotient;
    quotient = NumbersOperations.concatenateTwoNumbersToOne(quotient,
        quotientAndRemaining.quotient);
    ...
    divisionVectorOffsets.subtractionResultOffset = calculateSubtractionOffsettBasedOnRemain(
        dividend, divisor, lastNumBeforeRemainder, divisionVectorOffsets);
    ...
    if (nextDigitOfQuotient == 0) {
      ...
      result.add(new int[]{divisor * nextDigitOfQuotient, remain,
          divisionVectorOffsets.divisionResultOffset,
          divisionVectorOffsets.subtractionResultOffset});
    }
